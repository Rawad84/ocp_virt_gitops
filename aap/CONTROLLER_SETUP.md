...
install` to actually find it. See `aap/LESSONS_LEARNED.md` "Collections /
air-gapped" if there's no Hub reachable from wherever this runs.
...
## 4. Credential — `openshift-in-cluster`

Named `openshift-<cluster>` where `<cluster>` matches whatever `cluster`
extra_var a request uses — a different target cluster needs its own
separately-named credential, same pattern, no code change. Used in two
places, both by name, neither as a static Job Template attachment:

- The per-namespace dynamic inventory's `inventory_source` (see
  `ansible/playbooks/intake_create_vm_request.yml` header).
- Passed as `credentials: ["openshift-{{ cluster }}"]` at launch time to
  ...
  vary per launch, so a different `cluster` value would silently use the
  wrong cluster's credential.

RBAC/ServiceAccount to back this: `aap/rbac/vm-inventory-serviceaccount.yaml`
(apply with `oc apply -f`, generate token with
`oc create token aap-vm-inventory -n aap --duration=8760h`).

**Access → Credentials → Add**
- Name: `openshift-in-cluster`
- Organization: `Default`
...
does NOT need an `openshift-<cluster>` credential attached. See
...
- Extra variables:
  - Check **Prompt on launch** (top-right checkbox) — required, since
    `group_name`/`cluster`/`namespace`/`ssh_key`/`networks`/`vms` differ on
    every launch and have no defaults in the playbook
...
- Options → check **Prompt on launch** for both:
  ...
  - **Credentials** (lets the caller pass `openshift-{{ cluster }}` by name
...
- Options → check **Prompt on launch** for both:
  - **Credentials** (`openshift-{{ cluster }}` by name at launch time, same
